    ...
    for manager in managers {
        ...
        match manager.get_stats() {
            Ok(stats) => {
                ...
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_wg_len_128={}",
                    d.tc_dbg_type4_wg_len_128
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_dynamic_peer={}",
                    d.tc_dbg_type4_dynamic_peer
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_fixed_peer={}",
                    d.tc_dbg_type4_fixed_peer
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_metadata_attempt={}",
                    d.tc_dbg_type4_metadata_attempt
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_metadata_success={}",
                    d.tc_dbg_type4_metadata_success
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_metadata_fail={}",
                    d.tc_dbg_type4_metadata_fail
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_after_final_header_byte8_is_06={}",
                    d.tc_dbg_type4_after_final_header_byte8_is_06
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_after_final_header_byte8_not_06={}",
                    d.tc_dbg_type4_after_final_header_byte8_not_06
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_store_offset_last={}",
                    d.tc_dbg_type4_store_offset_last
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_wg_len_last={}",
                    d.tc_dbg_type4_wg_len_last
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_len_code_last={}",
                    d.tc_dbg_type4_len_code_last
                );
                let _ = writeln!(
                    buf,
                    "{peer}_tc_dbg_type4_final_byte8_last={}",
